...
- Using a function as the value of a comparison operator like `<`, `>`, `=`, `!=`, `between` — **wrong**. Functions are not values; they are standalone conditions used with `"o": "f"`. For example, this is invalid:
    ...
    Use the function operator — the function already encodes the comparison (`isPast` means "column is in the past"):

    ```json
    { "c": "due_date", "o": "f", "v": { "fn": "isPast" } }
    ```

    To combine a dynamic-date check with another condition, wrap both as separate items in an `and` group:
